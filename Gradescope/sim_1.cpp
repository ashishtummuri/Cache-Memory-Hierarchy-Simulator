...
void Print_results(){
    ...
    //     << "\t\t" << (result[0].readmiss + result[0].writemiss)/(result[0].read + result[0].write)<< "\n";
    std::cout << "e. L1 Miss Rate:"
        << "\t\t" << static_cast<float>(result[0].readmiss + result[0].writemiss) / static_cast<float>(result[0].read + result[0].write) << "\n";

    std::cout << "f. Writebacks to L2 level:"
        << "\t" << result[1].writeback << "\n";
    ...
    /**************MEMORY*********************/
    std::cout << "o. Writebacks to MEM level:"
        << "\t" << result[2].writeback << "\n";
    std::cout << "p. L2 prefetch:"
        << "\t" << " " << "\n";
    std::cout << "q. MEM traffic:"
        << "\t\t\t" << result[2].traffic << "\n";
}
...
void init_Buffer(uint64_t address,int lru, CacheParams &params ){
    if(lru == -1){
        for(int i =0 ; i < params.PREF_N ;i++){
...
para getPara(uint64_t address, uint64_t SIZE, uint64_t ASSOC, uint64_t BLOCKSIZE){
    para temp;
    ...
    while (getline(file, line)) {
        ...
        if (iss >> rw >> std::hex >> addr) {
            if (rw == 'r' || rw == 'w') {
                ...
                uint64_t offset = addr & ((1 << OFFSET) - 1);
                uint64_t index = (addr >> OFFSET) & ((1 << INDEX) - 1);
                uint64_t tag = addr >> (OFFSET + INDEX);
...
void writeBack(Cache_flags** cache, int index, uint64_t address, uint64_t SIZE, uint64_t ASSOC, uint64_t BLOCKSIZE, CacheParams &params){
...
void readCache(Cache_flags** cache_R, int index, char rw, uint64_t address, uint64_t SIZE, uint64_t ASSOC, uint64_t BLOCKSIZE, CacheParams &params){
    ...
    for (int i = 0; i < ASSOC; i++) {
        // Case 1: tag found and dirty bit is 0
...
void writeCache(Cache_flags** cache_R, int index, char rw, uint64_t address, uint64_t SIZE, uint64_t ASSOC, uint64_t BLOCKSIZE, CacheParams &params){
...
void printCache(CacheParams &params){
    const int SETS_L1 = params.L1_SIZE / (params.L1_ASSOC * params.BLOCKSIZE);
    const int SETS_L2 = params.L2_SIZE / (params.L2_ASSOC * params.BLOCKSIZE);
    ...
    for(int j=0;j< SETS_L1;j++){ 
        for (int i = 0; i < params.L1_ASSOC; i++) {
            if(cache[j][i].tag != 0)
                std::cout<<j<<"!!!!TAG: "<<cache[j][i].tag<<" VALID: "<<cache[j][i].valid<<" DIRTY: "<<cache[j][i].dirty<<" LRU: "<<cache[j][i].lru<<std::endl;
            // else
